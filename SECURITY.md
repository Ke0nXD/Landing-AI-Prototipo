...
AI Landing Lab can run with local mock generation or a real server-side AI provider such as Anthropic Claude. The security boundary is shaped so provider secrets and model calls stay on the server.

## Current Controls

- User prompt input is validated with Zod in `lib/validators/landing.ts`.
...
- AI provider calls live in `lib/ai/generate.ts` and are marked server-only.
- Anthropic credentials are read from `ANTHROPIC_API_KEY` only on the server.
- Model output is parsed as JSON, validated, normalized, sanitized, and then rendered as text.
...
- Do not expose `ANTHROPIC_API_KEY`, `HF_API_TOKEN`, `SUPABASE_SERVICE_ROLE_KEY`, or other provider secrets with a `NEXT_PUBLIC_` prefix.
- Keep AI generation behind server actions or route handlers.
- Store only prompt and business data that the user intentionally submits.
